...
let currentVolumePercentage = 100;
...
function initializeRequestListeners() {
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
            ...
            if (request.message === "change_volume") {
                adjustVolume();
            }
            else if (request.message === "save_volume"){
                console.log("Saving volume: " + request.value);
                currentVolumePercentage = Number(request.value);
                sessionStorage.setItem(VOLUME_KEY, currentVolumePercentage);
            }
            else if (request.message === "get_volume"){
                sendResponse(currentVolumePercentage);
            }
            else if (request.message === "reset_volume"){
                resetVolume();
                adjustVolume();
            }
            else if (request.message === "restore_volume"){
                setTimeout(function(){
                    gainNode = gainNode || firstTimeSetUp();

                    if (gainNode) {
                        let volumeMultiplier = Number(sessionStorage.getItem(VOLUME_KEY)) / 100;
                        if (0 <= volumeMultiplier && volumeMultiplier <= 10) {
